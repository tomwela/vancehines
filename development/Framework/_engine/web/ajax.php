...
class Ajax
{
   ...
   {
      $actions = implode(";\n", $this->actions['top']) . ((count($this->actions['bottom'])) ? ";\n" : '') . implode(";\n", $this->actions['bottom']);
      if ($actions != '') $actions .= ";\n" . $this->parent . 'ajax.initViewStates(true)';
      if (Ajax::isAction())
      {
   ...
   public function addAction($action, $time = 0, $place = false)
   {
   ...
   public function tool($tool, $time = 0, $place = false, $replace = false)
   {
      return $this->addAction($this->parent . 'Ajax.action(\'tool\', \'' . $this->replaceBreakup($tool) . '\', ' . ($replace?1:0) . ')', $time, $place);
   }

   public function css($src, $time = 0, $place = false, $replace = false)
   {
      return $this->addAction($this->parent . 'Ajax.action(\'css\', \'' . $this->replaceBreakup($src) . '\', ' . ($replace?1:0) . ')', $time, $place);
   }
  ...
  {
    return $this->display($id, $class);
  }

  public function hide($id)
  {
    return $this->display($id, 'none');
  }

  public function displayBool($id, $bool, $class = '')
  {
    return $bool ? $this->show($id, $class) : $this->hide($id);
  }
   ...
   public function runScript($script, $time=0, $place=false)
   {
      if (!self::isAction()) 
      {
        if($time>0) $script = "{$this->parent}setTimeout(function(){ $script }, {$time});";
        JS::getInstance()->add(new Helpers\Script(uniqid("script"), $script, null));
        return $this;
      }
      else return $this->addAction($script, $time, $place);
    }
   ...
   public static function ajaxArray(array $obj)
   {
      ...
      foreach ($obj as $k => $v)
      {
        switch(true)
        {
          case     (is_bool($v)): $v = (int)$v;                           break;
          case   (is_object($v)): $v = "'" . self::ajaxObject($v) . "'";  break;
          case    (is_array($v)): $v = self::ajaxArray($v);               break;
          case (!is_numeric($v)): $v = "'" . addslashes($v) . "'";        break;
        }
        $js[] = (is_int($k)) ? $v : "'" . addslashes($k) . "': " . $v;
      }
   ...
   {
    return self::isAction() ? strtr($str, array("\r" => '&#0013;', "\n" => '&#0010;')) : $str;
   }
