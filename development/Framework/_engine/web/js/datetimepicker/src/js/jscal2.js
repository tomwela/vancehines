...
a=U(a,{
fps:50,len:15,onUpdate:bl,onStop:bl}
),e&&(a.len=Math.round(a.len/2)),f();
return{
start:f,stop:g,update:h,args:a,map:d}
}
function bc(a,b){
if(!b(a))for(var c=a.firstChild;
c;
c=c.nextSibling)c.nodeType==1&&bc(c,b)}
function bb(a,b){
...
return!1}
function W(a,b,c,d){
if(a instanceof Array)for(var f=a.length;
--f>=0;
)W(a[f],b,c);
...
b.left=c.x+"px",b.top=c.y/*(c.y<0?0:c.y)*/+"px"}
function A(b,c){
c=c||window.event;
var d=C(c);
...
,this.args=b=U(b,{
animation:!f,cont:null,bottomBar:!0,date:!0,fdow:L("fdow"),min:null,max:null,reverseWheel:!1,selection:[],selectionType:a.SEL_SINGLE,weekNumbers:!1,align:"Bl/ / /T/r",inputField:null,trigger:null,dateFormat:"%m-%d-%Y",fixed:!1,opacity:e?1:3,titleFormat:"%b %Y",showTime:!1,timePos:"right",time:!0,minuteStep:5,noScroll:!1,disabled:bl,checkRange:!1,dateInfo:bl,onChange:bl,onSelect:bl,onTimeChange:bl,onFocus:bl,onBlur:bl}
),this.handlers={
}
;
...
,h.showAt=function(a,b,c)
{
  ...
  h.position="absolute",h.left=a+"px",h.top=b/*((b>0)?b:0)*/+"px",h.zIndex=1e4,h.display="",c&&(f.style.marginTop=-g+"px",this.args.opacity>1&&bf(d,0),
  this._showAnim=bd(
  {onUpdate:function(a,b){f.style.marginTop=-b(be.accel_b(a),g,0)+"px",e.args.opacity>1&&bf(d,a)},
   onStop:function(){e.args.opacity>1&&bf(d,""),e._showAnim=null}
  }
  ))
}
,h.hide=function(){
...
function h(b)
{
  var c={x:i.x,y:i.y};
  if(!b)return c;
  /B/.test(b)&&(c.y+=a.offsetHeight),
  /b/.test(b)&&(c.y+=a.offsetHeight-f.y),
  /T/.test(b)&&(c.y+=a.offsetHeight),//(c.y-=f.y),
  /l/.test(b)&&(c.x-=f.x-a.offsetWidth),
  /L/.test(b)&&(c.x-=f.x),
  /R/.test(b)&&(c.x+=a.offsetWidth),
  /c/i.test(b)&&(c.x+=(a.offsetWidth-f.x)/2),
  /m/i.test(b)&&(c.y+=(a.offsetHeight-f.y)/2),
  1;
  return c
}
a=bm(a),
...
f={x:d.offsetWidth,y:d.offsetHeight};
  ...
  var i=c;
  i=h(b[0]),
  i.y<g.y&&(i.y=c.y,i=h(b[1])),
  i.x+f.x>g.x+g.w&&(i.x=c.x,i=h(b[2])),
  i.y+f.y>g.y+g.h&&(i.y=c.y,i=h(b[3])),
  i.x<g.x&&(i.x=c.x,i=h(b[4])),
...
var e=this;
c=bm(c),b=bm(b),(b&&b.tagName&&b.tagName.match(/^button$/i))&&b.setAttribute("type","button"),V(b,"click",function(){
...
for(;d<c.length;++d)if(typeof c[d]!=="undefined")c[d].apply(this,b);else console.log(this.handlers);}
...
a=O(a);
var c=!1;
for(var d=this.sel,e=d.length,f;
--e>=0;
){
f=d[e];
...
for(var a=this.sel,b=a.length,c,d;
--b>=0;
){
c=a[b];
...
for(var b=this.sel,c=b.length,d;
--c>=0;
){
