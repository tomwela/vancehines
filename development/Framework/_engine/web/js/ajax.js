var Ajax = function()
{
   ...
   this.convertToObject = function(param, iter)
   {
      if(typeof iter === 'undefined')iter = 2;
      if (typeof(param) != 'object') return param;
      var arr = {};
      for (var i in param)
      {
         if (typeof(param[i]) == 'object' && iter>0 && (typeof param[i]!== 'Window')) arr[i] = this.convertToObject(param[i], iter-1);
         else arr[i] = param[i];
      }
      return arr;
   };
   ...
   this.getControlValues = function(flag)
   {
      ...
      for (var i = 0; i < elements.length; i++)
      {
         ...
         // var value;
         // var type = el.type ? el.type : el.attributes['type'];
         switch (type)
         {
            default:
            continue;
   ...
   this.getFormElements = function(el, tags)
   {
      ...
      if (!tags) tags = 'input,select,textarea';
      tags = tags.split(',');
      var elements = [];
...
{
  arguments[1] = arguments[1].replace(/\&\#0010\;/g, "").replace(/\&\#0013\;/g, "");
   switch (arguments[0])
   {
      case 'alert':
        alert(arguments[1]);
      ...
      case 'tool':
        ...
        el.type = 'text/javascript';
        var links = head.getElementsByTagName('SCRIPT');
        replace = false;
        for (i = 0; i<links.length; i++) 
          if (links.item(i).getAttribute('src') && links.item(i).getAttribute('src').split('?')[0] == arguments[1].split('?')[0]) 
        {
          if(typeof arguments[2] !=='undefined') replace = arguments[2];
          if(replace) { p=links.item(i).parentNode;p.removeChild(links.item(i)); }
          else return;
        }
        head.appendChild(el);
        el.setAttribute('src', arguments[1] + (replace?('?' + (new Date()).getTime()):''));
        break;
      ...
      case 'css':
        ...
        replace = false;
        for (i = 0; i<links.length; i++) 
          if (links.item(i).getAttribute('href') && links.item(i).getAttribute('href').split('?')[0] == arguments[1]) 
        {
          if(typeof arguments[2] !=='undefined') replace = arguments[2];
          if(replace) { p=links.item(i).parentNode;p.removeChild(links.item(i)); }
          else return;
        ...
        head.appendChild(el);
        el.setAttribute('href', arguments[1] + (replace?('?' + (new Date()).getTime()):''));
   }
};
