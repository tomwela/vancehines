...
class Panel extends WebControl implements IPanel
{
   ...
   {
      $attributes['template'] = $attributes['template'] ?: $this->properties['template'];
   ...
   public function setTemplate($template, Core\ITemplate $tpl = null)
   {
      ...
      $this->page->invokeMethod('init', $this->attributes['uniqueID']);
      return $this;
   }
   ...
   {
      $controls = array();
      foreach ($this->controls as $uid => $v) 
      {
         $ctrl = $this->getByUniqueID($uid);
         if($ctrl instanceof Panel) $controls= array_merge($controls,$ctrl->getAllControls());
         $controls[$uid]=$v;
      }
      return $controls;
   }
   ...
   public function assign($values, $isRecursion = false, $isRecursionAssignment = false)
   {
   ...
   {
      $tag = $this->reg->config['templateControlPrefix'].strtolower(Utils\PHPParser::getClassName(get_class($this)));
      // TODO: fix this temp hack (double rendering)
      //if($this->properties['template']) is not solution 
      if(!isset(Web\XHTMLParser::$controls[strtoupper(Utils\PHPParser::getClassName(get_class($this)))])) return '<' . $tag . $this->getXHTMLParams() . '/>';
   ...
   public function render()
   {
      if($this->properties['visible'])
      {
         ...
         $this->tpl->parentUniqueID = $this->parentUniqueID;
      }
      ...
      else $html = '<' . $this->properties['tag'] . $this->getParams() . '>' . $this->getInnerHTML()  . '</' . $this->properties['tag'] . '>';
