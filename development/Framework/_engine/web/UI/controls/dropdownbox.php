...
class DropDownBox extends WebControl
{
   ...
   public function __set($param, $value)
   {
      ...
      if ($param == 'options' && is_array($value)) 
      {
         $this->properties['options'] = $value;
         if(!isset($value[$this->properties['value']]))
            $this->properties['value']   =  $this->properties['defaultValue'] ? 0 : key($value);
      }
   ...
   public function getOptionsHTML()
   {
      ...
      if ($this->properties['defaultValue'] != '') $ops = array(false => $this->properties['defaultValue']) + $ops;
