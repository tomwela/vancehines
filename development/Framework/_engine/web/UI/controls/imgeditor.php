...
class ImgEditor extends Panel
{
   public function __construct($id)
   {
      ...
      $this->properties['template'] = $this->reg->config->root . '/Framework/_engine/web/js/imgeditor/imgeditor.html';
   }
   ...
   public function editImage($uniqueID)
   {
      ...
      $info =end(Utils\UploadFile::getInfo($uniqueID));
      $iem = 'iem_' . $this->attributes['uniqueID'];

      $this->get('btnUseOriginal')->onclick = $this->method('useOriginalImage', array($uniqueID, 1));
      $this->get('btnUseThisImage')->onclick = 'controls.$("'.$this->get('btnCancel')->uniqueID.'").disabled="disabled";this.disabled="disabled";' . $this->method('useNewImage', array($uniqueID, 'js::' . $iem . '.edt.options.angle', 'js::' . $iem . '.edt.options.scale', 'js::' . $iem . '.edt.getCropParameters()', 1));
   ...
   public function uploadImage($uniqueID)
   {
      ...
      $file->mode = Utils\UploadFile::UPLOAD_MODE_TEMP_MULTIPLY;
      $file->maxsize = (isset($params['maxSize'])) ? $params['maxSize'] : MAX_PICTURE_SIZE;
      $file->unique = true;
      ...
      if (($info = $file->upload((array)$this->page->fv[$uniqueID])) !== false)
      {
         list($width, $height, $type, $attr) = getimagesize($info['fullname']);
         ...
         $this->get('btnUseThisImage')->onclick = 'controls.$("'.$this->get('btnCancel')->uniqueID.'").disabled="disabled";this.disabled="disabled";' . $this->method('useNewImage', array($uniqueID, 'js::' . $iem . '.edt.options.angle', 'js::' . $iem . '.edt.options.scale', 'js::' . $iem . '.edt.getCropParameters()'));
   ...
   public function useNewImage($uniqueID, $angle, $scale, $cropInfo, $isEdit = false)
   {
      ...
      $info = end($info);
      if (!is_file($info['fullname'])) return;
      list($width, $height, $type, $attr) = getimagesize($info['fullname']);
      $scale = $scale / 100;
      $this->ajax->script('iem_' . $this->attributes['uniqueID'] . '.hide()');
      $this->scale($info['fullname'], $info['fullname'], $width * $scale, $height * $scale);
      $this->rotate($info['fullname'], $info['fullname'], $angle, $this->get('backgroundColor')->value);
      if (!$params['hideCrop']) $this->crop($info['fullname'], $info['fullname'], $cropInfo['x'], $cropInfo['y'], $cropInfo['width'], $cropInfo['height'], $this->get('backgroundColor')->value, $this->get('isSmartCrop')->checked);
      ...
      $this->get('btnUseThisImage')->onclick = 'this.disabled="disabled";' . $this->method('useNewImage', array($uniqueID, 'js::' . $iem . '.edt.options.angle', 'js::' . $iem . '.edt.options.scale', 'js::' . $iem . '.edt.getCropParameters()', 1));
   }
   ...
   private function rotate($src, $dest, $angle, $bgcolor)
   {
      foo(new Utils\Picture($src))->rotate($dest, $angle, $bgcolor);
   }

   private function crop($src, $dest, $left, $top, $width, $height, $bgcolor, $isSmartCrop)
   {
      foo(new Utils\Picture($src))->crop($dest, $left, $top, $width, $height, $bgcolor, $isSmartCrop);
   }
}
