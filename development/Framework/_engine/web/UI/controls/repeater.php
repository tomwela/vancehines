...
class Repeater extends Panel
{
  ...
  protected function setCount($count)
  {
    ...
    elseif ($count > $this->properties['count']) for ($i = $this->properties['count'] + 1; $i <= $count; $i++) $this->put($i);
  }
  ...
  protected function getXHTMLParams()
  {
    ...
    unset($this->properties['source']);
    unset($this->properties['sections']);
  ...
  public function getSectionByUID($uid)
  {
    foreach ($this->properties['order'] as $n)
      foreach ($this->properties['sections'][$n] as $uniqueID)
        if($uniqueID == $uid) return $n;
    return false;
  }

  public function getByIndex($id, $index)
  {
    return parent::get("{$id}_{$index}");
  }

  public function getFirst($id)
  {
    return $this->getByIndex($id, 0);
  }
  public function getLast($id)
  {
    return $this->getByIndex($id, $this->count-1);
  }
}
