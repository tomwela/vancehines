...
class URI
{
   ...
   {  
	/** 
	* Ruben 07/22/2014
	* I don't think this framework worked with https URLs properly.  Logging in to the https link would then redirect to a http
	* page causing browser issues and breaking security intentions.
	* I replaced $_SERVER['SERVER_PROTOCOL'] test with a test for $_SERVER['HTTPS'] to determine what protocol the site is using
	* and continue using the same protocol going forward.
	* 
	*/
	// original code from Saratossa
   ...
   public function getURI()
   {
      if ($this->scheme == '')
	  {
	     return $this->getSource() . $this->getQueryString();
	  }
      switch ($this->scheme)
      {
         case 'http':
         case 'https':
           return $this->scheme . '://' . $this->getSource() . $this->getQueryString();
      }
   }
