...
class Picture
{
   ...
   public function rotate($dest, $angle, $bgcolor = '#ffffff')
   {
      ...
      if (function_exists('NewMagickWand'))
      {
         ...
         if ($alpha[0] == 0) // transparent image
         {
            $pw = NewPixelWand();
   ...
   public function crop($dest, $left, $top, $width, $height, $bgcolor = '#ffffff', $isSmartCrop = false)
   {
      $dest = Core\IO::dir($dest);
      if ($bgcolor[0] != '#') $bgcolor = '#' . $bgcolor;
      if (function_exists('NewMagickWand'))
      {
         ...
         if ($magickVersion[1] >= 1589)
         {
            ...
            MagickSetImageBackgroundColor($this->img, NewPixelWand($bgcolor));
      ...
      else
      {
         ...
         imageCopy($img, $this->img, $l, $t, $srcDimensions['left'], $srcDimensions['top'], $srcDimensions['width'], $srcDimensions['height']);
  ...
  public static function imagettftextSp($image, $size, $angle, $x, $y, $color, $font, $text, $spacing = 0)
  {        
      if ($spacing == 0)
      {
  ...
  public static function createImageFromFile($src)
  {
      ...
      try
      {
         switch (strtolower($info['extension']))
         {
            ...
            case 'jpg':
              ...
              break;
            default: // if extension empty
              try { return imagecreatefromgif ($this->src); } catch(\Exception $e) {
              try { return imagecreatefromjpeg($this->src); } catch(\Exception $e) {
              try { return imagecreatefrompng ($this->src); } catch(\Exception $e) { return false; }}}
         }
      }
      catch(\Exception $e) { 
        try { return imagecreatefromgif ($this->src); } catch(\Exception $e) {
