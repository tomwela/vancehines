...
class DALTable implements IDALTable, \Serializable
{
   ...
   public function __set($field, $value)
   {
      ...
      {
         if ($value !== $this->info['fields'][$field]['value'] && $this->info['fields'][$field]['autoincrement']) throw new \Exception(err_msg('ERR_DAL_2', array($field)));
      }
      if (is_object($value)) throw new \Exception(err_msg('ERR_DAL_1', array($field)));
   ...
   public function assign(array $data = null)
   {
      if (is_array($data) && count($data))
      {
         foreach ($data as $k => $v)
         {
            ...
            if (!$field) continue;
            if (strlen($v) == 0) $this->info['fields'][$field]['value'] = $this->info['fields'][$field]['default'];
            else $this->info['fields'][$field]['value'] = $v;
   ...
   public function insert()
   {
      ...
      {
         if ($this->isKeyFilled()) throw new \Exception(err_msg('ERR_DAL_5', array($this->getTable())));
         $this->info['fields'][end($this->info['pk'])]['value'] = $this->db->insert($this->getTable(), $this->getData());
      }
      else
      {
         if (!$this->isKeyFilled()) throw new \Exception(err_msg('ERR_DAL_6', array($this->getTable())));
   ...
   public function update()
   {
      if (!$this->isKeyFilled()) throw new \Exception(err_msg('ERR_DAL_6', array($this->getTable())));
      if ($this->isUpdated) 
         $res = $this->db->update($this->getTable(), $this->getData(), $this->getKeyData());
      $this->isUpdated = false;
      return (int)$res;
   }
