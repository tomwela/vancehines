...
class ORMGenerator
{
      ...
      foreach (ORM::getInstance()->getDBList() as $db)
      {
         ...
         foreach ((array)$info[$dbName]['tables'] as $table => $data)
         {
            ...
            foreach ((array)$info[$dbName][$mode][$table] as $dd)
            {
               ...
               {
                  if ((int)$dd['multiplicity'] == 0) $nodeName = 'parent' . ucfirst($nodeName) . ++$pc;
                  else $nodeName = 'children' . ucfirst($nodeName) . ++$cc;
               }
            ...
            foreach ($data['fields'] as $field)
            {
               ...
               if (strlen($field['DefaultValue'])) $nodeField->setAttribute('Default', htmlspecialchars($field['DefaultValue']));
   ...
   public function generateFiles()
   {
      ...
      foreach ($this->info['model'] as $dbalias => $dbs)
      {
         foreach ($dbs['tables'] as $table => $data)
         {
            ...
            {
               $properties[$fieldAlias] = ' * @property ' . $info['type'] . ' $' . $fieldAlias;
      ...
      foreach ($this->info['classes'] as $class => $data)
      {
         ...
         {
            $k = strrpos($className, '\\');
            if ($k !== false) $className = substr($className, $k + 1);
            $db = $this->info['classes'][$className]['db'];
            $tb = $this->info['classes'][$className]['table']['alias'];
            $properties[$field] = ' * @property ' . $this->info['model'][$db]['tables'][$tb]['fields'][$field]['type']. ' $' . $field;
         }
         foreach ($data['table']['logicFields'] as $field => $className)
         {
            $properties[$field] = ' * @property custom $' . $field;
         }
         foreach (array('procedures', 'functions') as $type) foreach ((array)$this->info['model'][$dbalias]['tables'][$table][$type] as $sp)
         {
           $db = ORM::getInstance()->getDB($dbalias);
           $args = $db->getRoutine($sp['link'], $type == 'functions');
           $args = $args['args']; $tmp1 = $tmp2 = array();
           foreach ($args as $arg => $t) 
           {
             $tmp1[] = '$' . $arg;
             $tmp2[] = '?';
           }
           $method = '  public static function ' . $sp['name'] . '(' . implode(', ', $tmp1) . ')' . endl . '  {' . endl . '    ';
           $method .= ($type == 'functions' ? 'return ' : '') . '\ClickBlocks\DB\ORM::getInstance()->getDB(\'' . $dbalias . '\')->col(\'' . ($type == 'functions' ? 'SELECT' : 'CALL') . ' ' . $db->wrap($sp['link']) . '(' . implode(', ', $tmp2) . ')\', array(' . implode(', ', $tmp1) . '));';
           $method .= endl . '  }' . endl;
           $methods[$sp['name']] = $method;
         }
         foreach ($data['table']['navigationFields'] as  $field => $className)
         {
            $info = $this->info['model'][$dbalias]['tables'][$table]['navigationFields'][$field];
            $properties[$field] = ' * @property navigation $' . $field;
   ...
   protected function info()
   {
      ...
      foreach (ORM::getInstance()->getDBList() as $alias => $db)
      {
         ...
         foreach ($tables as $table)
         {
            ...
            foreach ($info[$dbName]['tables'][$table]['fields'] as $field => $v) if ($v['PK']) $info[$dbName]['tables'][$table]['key'][$field] = $field;
            if (preg_match_all('/FOREIGN KEY \(`([^)]+)`\) REFERENCES `([^`]+(`.`)?[^`]+)` \(`([^)]+)`\)/is', $ddl, $key))
            {
               $cmp = function($a,$b) { return strcasecmp($a,$b); };
               uasort($key[2], $cmp);
               foreach ($key[2] as $n => $ddl)
               {
                  ...
                  $info[$toDB]['foreignkeys'][$toTable][] = array('toDB' => $dbName, 'toTable' => $table, 'toFields' => $fromFields, 'fromFields' => $toFields);
                  $info[$dbName]['dependences'][$table][] = array('toDB' => $toDB, 'toTable' => $toTable, 'toFields' => $toFields, 'fromFields' => $fromFields);
               }
            }
         }
   ...
   {
      $tmp = explode('_', ($table));
      foreach ($tmp as $k => $v) if (trim($v)) $tmp[$k] = ucfirst(trim($v)); else unset($tmp[$k]);
      return implode('', $tmp);
   }
   ...
   private function rewriteClass($file, Core\Template $tpl)
   {
      ...
      foreach ($tplInfo->{$tplClass}->methods as $method => $methodInfo)
      {
         ...
         else
         {
            ...
            {
               $info->{$fullClassName}->methods[$method]->code = $needCode . $code;
            }
         }
      }
