 ...
 *
 * Responsibility of this file: reader.php
 *
 * @category   DB
 * @package    DB
...
class MySQLReader extends DBReader
{
  ...
  public function read()
  {
    ...
    foreach ($tables as $table => $meta)
    {
      ...
      {
        $tableSQL = $this->db->getSQL('data', 'table', array('tbl_name' => $tblName));
        $st = $pdo->prepare($tableSQL);
        $st->execute();
  ...
  protected function getData(\PDO $pdo, $type, array $params = null)
  {
    ...
    else if ($type == 'columns')
    {
      while ($row = $st->fetch(\PDO::FETCH_ASSOC)) 
      {
        ...
        $def .= $default = strlen(trim($row['Default'])) ? ' DEFAULT ' . ((substr($row['Type'], 0, 3) == 'bit' || $row['Default'] == 'CURRENT_TIMESTAMP') ? $row['Default'] : $this->db->quote($row['Default'])) : '';
        $def .= $row['Extra'] == 'auto_increment' ? ' AUTO_INCREMENT' : '';
        $def .= substr($row['Extra'], 0, 2) == 'on' ? ' ' . $row['Extra'] : '';
        $def .= strlen(trim($row['Comment'])) ? ' COMMENT ' . $this->db->quote($row['Comment']) : '';
        $tmp = array();
    ...
    {
      echo "Type $type is not recognized";
    }
    return $rows;
  }
}