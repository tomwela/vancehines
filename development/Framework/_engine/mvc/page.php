...
class Page implements IPage
{
   ...
   public function process()
   {
      ...
      ob_start();
      $res = $func->call($this->fv['ajaxargs']);
      $this->ajax->script("ajax.result = '$res'");
   ...
   public function unload()
   {
      ...
      if($this->config->dbStatisticDebug)
      {
        $st = \ClickBlocks\DB\DB::getStatistic();
        foreach($st as $db=>$stat)
        {
          $res[$db] = array();          
          foreach ($stat as $key => $value) $res[$db][$key.': '.Utils\Strings::cut($value['sql'], 30)] = $value;
        }
        \ClickBlocks\Web\Ajax::getInstance()->consolelog($res);
      }
   }
   ...
   public function invokeMethod($method, $uniqueID)
   {
      $vs = $this->getActualVS($uniqueID);
      if ($vs['controls'])
      {
