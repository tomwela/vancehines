...
class Debugger
{
   ...
   public static function exceptionHandler(\Exception $e, $category = Logger::LOG_CATEGORY_EXCEPTION)
   {
      ...
      {
        if(is_array($trace[0]['args'][1]))$trace[0]['args'][1] = implode('; ', $trace[0]['args'][1]);
         if (preg_match('/on line (\d+)/', $trace[0]['args'][1]))
         {
    ...
    * @static
    */
   private static function errorMessage($category, $stack, $error, $file, $line, $fragment, $isFatal = false, $stop = true)
   {
      ...
      if (php_sapi_name() === 'cli')
      {
         ...
         if (!is_file(IO::dir($config->debugPage)))
         {
            ...
            self::emailSend(self::$output);
            return;
         }

      self::emailSend(strtr(file_get_contents(IO::dir($config->debugPage)), $vars));
      if ($config->isDebug)
      {
  ...
  private static function emailSend($body)
  {
    // echo "<<<MAIL\n".$body."\nMAIL>>>";return;
    $config = Register::getInstance()->config;
    if(!$config->debugEmails)return;
    $email = explode(',', $config->debugEmails);
  
    $mailer = new \PHPMailer();
    $mailer->Subject = 'UG Bug Report';
    $mailer->Body = $body;
    $mailer->isHTML(true);
    $mailer->FromName = 'UltraGoods Debugger';
    $mailer->From = 'debugger@'.$_SERVER['SERVER_NAME'];
    if (is_array($email))
    {
      foreach ($email as $e)
        $mailer->AddAddress($e);
    }
    else $mailer->AddAddress($email);
    return $mailer->Send();  
  }
   ...
   private static function redirect($url, $isNewWindow = false)
   {
      if ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')
      {
         ...
         return 'window.location.assign(\'' . addslashes($url) . '\')';
      }
      else
      {
