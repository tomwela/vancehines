...
class Template implements ITemplate
{
   ...
   public function setTemplate($key, $template, $parent = null, $var = null)
   {
      if ($key == '') throw new \Exception(err_msg('ERR_TEMPLATE_2'));
      if ($parent !== null)
      {
         if (!isset($this->templates[$parent])) throw new \Exception(err_msg('ERR_TEMPLATE_3', array($parent)));
   ...
   public function render($key = null)
   {
      ...
      ${'|template|'} = $this->templates[$key]['template'];
      ${'|self|'} = (array)$this->vars[$key];
      foreach ((array)$this->templates[$key]['sibs'] as $k => $var)
      {
         if (isset($this->vars[''][$var]) || isset($this->vars[$key][$var])) continue;
         $$var = $this->render($k);
      }

      extract((array)$this->vars[$key], EXTR_SKIP); //self tpl

      ${'|vars|'} = (array)$this->vars[$key];
       while(${'|vars|'}['parentUniqueID'] && ${'|vars|'} = $this->vars[${'|vars|'}['parentUniqueID']] )
         extract(${'|vars|'}, EXTR_SKIP);

      extract((array)$this->vars[''], EXTR_SKIP); //page tpl
      ob_start();
      if (is_file(${'|template|'})) require(${'|template|'});
      else
      {
         Debugger::setEvalCode(${'|template|'});
         if (eval(' ?>' . ${'|template|'} . '<?php ') === false) Debugger::setEvalCode(${'|template|'}, true);
      }
