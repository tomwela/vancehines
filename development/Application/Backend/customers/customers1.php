...
#    $popup->show();
    ...
    public function copyTableFromMapID($valKey, $sourceMap)
    {
        ...
        //  checks
        if (!$sourceMap) {
            $this->ShowPopupMessage('Enter a map ID to copy from!');

            return;
        } elseif (!is_numeric($sourceMap)) {
            $this->ShowPopupMessage('Map ID should be an Integer!');

            return;
        }

        // does the sourceMap exist? returns the id.
        $mapID = foo(new DB\OrchestraMaps())->doesMapExist($sourceMap);
        if (!$mapID) {
            $this->ShowPopupMessage('This map ID does not exist!');

            return;
        }

        // process the source current map to copy from
        $m = foo(new DB\ServiceMaps())->getByID($mapID);
        $mStructure = getMapStructure( $this->customer->ecmFirmware );
        $orchestraNameFront = '\ClickBlocks\DB\Orchestra' . $mStructure[ $valKey ]['DBTableName'];

        $valueFront = foo(new $orchestraNameFront)->getByQuery('mapID = ' . $mapID, null, null, 'row');
        $dataFront = json_decode($valueFront['data'], 1);


        // s=source, count all elements of a multidimensional array
        $sNumbCols = count($dataFront);
        $sNumbRows = (count($dataFront, 1) - $sNumbCols) / $sNumbCols;

        //$this->ShowPopupMessage($sNumbRows . "x" . $sNumbCols);
        //return;

        // process this current map
        $cm = foo(new DB\ServiceMaps())->getByID($this->map->mapID);
        $cmStructure = getMapStructure( $this->customer->ecmFirmware );
        $cmOrchestraName = '\ClickBlocks\DB\Orchestra' . $cmStructure[ $valKey ]['DBTableName'];
        $valueCM = foo(new $cmOrchestraName)->getByQuery('mapID = ' . $this->map->mapID, null, null, 'row');
        $dataCM = json_decode($valueCM['data'], 1);


        // count all elements of a multidimensional array
        $thisNumbCols = count($dataCM);
        $thisNumbRows = (count($dataCM, 1) - $thisNumbCols) / $thisNumbCols;
        //$this->ShowPopupMessage($thisNumbRows ."x". $thisNumbCols);
        //return;


        // copy map only if it's of the same size
        if (($sNumbCols == $thisNumbCols) && ($sNumbRows == $thisNumbRows)) {
            if ($mStructure[ $valKey ]['dataType'] == 'matrix') {
                $popup = $this->get('mapSSInput');

                $popup->tpl->keyTable = $valKey;
                $popup->tpl->title = $mStructure[ $valKey ]['DBTableName'];

                $popup->tpl->rows = $dataFront;
                $popup->tpl->arrMatrixCol = array_keys($dataFront);
                $arrMatrixRow = array_values($dataFront);
                $popup->tpl->arrMatrixRow = array_keys($arrMatrixRow[0]);
                $popup->get('popupContainer')->get('valCols')->value = implode(',', $popup->tpl->arrMatrixCol);
            }
            ...
            #    $popup->show();
            #
            $this->ShowPopupMessage('Map was Copied!');

            return;
        } else {
            $this->ShowPopupMessage('The Source Map is a different size! ' . $sNumbRows . "x" . $sNumbCols);
   ...
   public function copyTableFromMapIDtables($valKey, $sourceMap)
    {
        /*  $valKey:  VEFrontCyl or VERearCyl
         *  $this->map->mapID    the map ID of the record being viewed (current map)
         *  $sourceMap  is the map entered into the input
        ...
        // count all elements of a multidimensional array
        $thisNumbRows = count($dataCM);



        // copy map only if it's of the same size
        if ($sNumbRows == $thisNumbRows) {
            if ($mStructure[ $valKey ]['dataType'] == 'table') {
               $popup = $this->get('mapMultiInput');
            ...
            $this->ShowPopupMessage('The Source Map is a different size! ' . $sNumbRows . "x" . $thisNumbRows);
