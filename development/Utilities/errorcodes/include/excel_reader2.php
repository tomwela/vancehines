   ...
   switch ($code) {
    case SPREADSHEET_EXCEL_READER_TYPE_SST:
     ...
     for ($i = 0; $i < $uniqueStrings; $i++) {
      ...
      $retstr = /*($asciiEncoding) ? iconv('cp1252','UTF-8//IGNORE', $retstr) :*/ $this->_encodeUTF16($retstr, $asciiEncoding);
...
//      $bgcolor = ($bgi & 0x3f80) >> 7;
 ...
 function _encodeUTF16($string, $ascii = false) {
  $result = $string;
  $encFrom = $ascii ? 'cp1252' : 'UTF-16LE';
  if ($this->_defaultEncoding){
   switch ($this->_encoderFunction){
    case 'iconv' :  $result = iconv($encFrom, $this->_defaultEncoding.'//IGNORE', $string);
        break;
    case 'mb_convert_encoding' :  $result = mb_convert_encoding($string, $this->_defaultEncoding, $encFrom );
        break;
   }
  }
  return $result;
 ...
 function xls2tstamp($date) {
  $date=$date>25568?$date:25569+$date;
